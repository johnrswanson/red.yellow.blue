...
if(isset($view))
	...
	echo'<a href="admin_new_album.php?albumID='.$view.'">+ New Album<P>';
	$data=mysql_query("select * from album where albumID='$view' order by albumorder ASC");
	$count=mysql_num_rows($data);
	
	if($count=='0')
		{echo'<P>You dont have any music albums yet. <P>';}
	else 
	...
	while($info=mysql_fetch_array($data))
		...
		echo'<a href="?editalbum='.$info['ID'].'"><img src="../img/album/thumbs/'.$info['photo'].'" style=" height:100px; float:left;';
		$path='../img/album/thumbs/'.$info['photo'].'';
